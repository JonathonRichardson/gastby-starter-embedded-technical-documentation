...
The `gatsby-config.ts` iterates over all the guides registered in the `content/guides.ts`, as well as the blog folder (which is hard coded there), and adds those folders with the `gatsby-source-filesystem`.  This makes these files available in graphql under the top level object `allFile`.
